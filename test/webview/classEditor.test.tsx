...
import { ClassEditor } from '../../src/webview/components/ClassEditor/ClassEditor';
...
describe('ClassEditor', () => {
  ...
  it('calls onChange with joined classes after debounce', () => {
    ...
    const { container } = render(
      <ClassEditor classes={['flex']} onChange={onChange} debounceMs={300} />
    );
    const textarea = container.querySelector('textarea')!;
  ...
  it('trims whitespace and filters empty lines when joining', () => {
    const onChange = vi.fn();
    const { container } = render(
      <ClassEditor classes={[]} onChange={onChange} debounceMs={100} />
    );
    const textarea = container.querySelector('textarea')!;
  ...
  it('resets debounce timer on rapid input', () => {
    const onChange = vi.fn();
    const { container } = render(
      <ClassEditor classes={[]} onChange={onChange} debounceMs={500} />
    );
    const textarea = container.querySelector('textarea')!;
  ...
  it('uses default debounceMs of 500', () => {
    const onChange = vi.fn();
    const { container } = render(
      <ClassEditor classes={[]} onChange={onChange} />
    );
    const textarea = container.querySelector('textarea')!;
