...
function createManager(editorText?: string, opts?: { cursorLine?: number }) {
  const editor = editorText
    ? createMockEditor(editorText, opts)
    : undefined;
...
describe('construction', () => {
  ...
  it('defaults to enabled (foldByDefault is true)', () => {
    const { manager, editor } = createManager('<div class="flex items-center p-4 rounded">');
...
<span class="flex items-center p-4 rounded">`;
    const { manager, editor } = createManager(text, { cursorLine: 0 });
