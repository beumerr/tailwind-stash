...
import { detectClassRanges } from '../src/classDetector';
...
describe('template literals with interpolations', () => {
  it('extracts static segments around interpolations', () => {
    const text = '<div className={`flex items-center gap-2 p-4 ${cond ? "a" : "b"} rounded mt-2 mb-2 mx-auto`}>';
    const results = detect(text);
...
describe('utility function calls', () => {
  ...
  it('handles nested parentheses', () => {
    const text = 'cn("flex items-center p-4 rounded", someCondition && "text-sm font-bold mt-2 mx-auto")';
    const results = detect(text);
    expect(results.length).toBeGreaterThanOrEqual(1);
  });
...
describe('Tailwind-specific class syntax', () => {
  ...
  it('handles state prefixes like hover: focus: dark:', () => {
    const results = detect('<div class="hover:bg-blue-500 focus:ring-2 dark:text-white active:scale-95">');
    expect(results).toHaveLength(1);
  });
  ...
  it('handles stacked modifiers like dark:hover:bg-blue-500', () => {
    const results = detect('<div class="dark:hover:bg-blue-500 sm:focus:ring-2 lg:dark:text-white group-hover:opacity-100">');
    expect(results).toHaveLength(1);
  });
});
...
describe('real-world JSX patterns', () => {
  ...
  it('handles className={cn("...", condition && "...")}', () => {
    const text = '<Button className={cn("flex items-center p-4 rounded", isActive && "bg-blue-500 text-white font-bold ring-2")}>';
    const results = detect(text);
    expect(results.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('handles two class attributes on the same line', () => {
    const text = '<div class="flex items-center p-4 rounded"></div><span class="text-sm font-bold mt-2 mx-auto"></span>';
    const results = detect(text);
    expect(results).toHaveLength(2);
  });
...
describe('boundary and stress scenarios', () => {
  ...
  it('handles many class attributes in a large document', () => {
    const lines = Array.from({ length: 20 }, (_, i) =>
      `<div class="flex items-center p-${i} rounded">`
    ).join('\n');
