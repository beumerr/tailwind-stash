...
export function detectClassRanges(
  ...
  // Deduplicate (html attr pattern may overlap with function pattern)
  return deduplicateRanges(results);
}
...
function addClassRange(
  ...
  if (classes.length < minClasses) return;
 ...
 */
function inferElement(document: vscode.TextDocument, offset: number, funcName: string | null): string {
