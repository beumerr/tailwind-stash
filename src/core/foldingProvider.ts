...
export class FoldingManager {
  ...
  constructor(private context: vscode.ExtensionContext) {
    ...
    this.disposables.push(
      ...
      vscode.window.onDidChangeTextEditorSelection((e) => {
        ...
        if (newLine === this.lastCursorLine) return;
  ...
  updateDecorations(editor: vscode.TextEditor) {
    ...
    const config = vscode.workspace.getConfiguration('tailwindStash');
    const supportedFunctions = config.get<string[]>('supportedFunctions', [
    ...
    editor.setDecorations(this.hideType, visibleRanges.map((cr) => ({ range: cr.range })));
  }
